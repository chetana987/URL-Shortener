...
| Area | Achievement |
|------|-------------|
| **Performance** | Sub-millisecond redirect resolution via Redis cache-aside pattern; 95% cache hit rate for active URLs |
| **Scalability** | Designed stateless JWT auth supporting horizontal scaling; implemented sliding-window rate limiting with atomic Lua scripts for 3 endpoint tiers |
| **Security** | BCrypt password hashing, JWT with refresh token rotation, user data isolation via JPA ownership queries, input validation on all 15 DTOs |
| **Architecture** | 5-entity relational schema with proper indexing; 8 service classes with single responsibility; composite caching strategy with tiered TTLs |
| **Developer UX** | Interactive Swagger/OpenAPI 3.0 documentation; standardized `ApiResponse<T>` with pagination; comprehensive global exception handling (9 custom exceptions) |
| **Frontend** | Vanilla JS with silent JWT auto-refresh, responsive Bootstrap 5 UI, real-time toast notifications, client-side token expiry detection |

### Key Technical Highlights

1. **Production-Ready Spring Boot**: Full REST API with validation, global exception handling, and standardized response format
2. **Redis-Driven Performance**: Cache-aside pattern with 30-min to 24-hour TTLs across 3 cache regions
3. **Stateless Auth with JWT**: Automatic token refresh with 60-second pre-emptive detection window
4. **Atomic Rate Limiting**: Redis sorted sets + Lua script providing thread-safe sliding window rate limits
5. **Comprehensive Analytics**: Track clicks, browsers, countries, referers, daily/hourly distributions with User-Agent parsing
6. **Full-Stack Implementation**: Vanilla JS frontend with auth guard, token management, and responsive design
7. **Docker-Ready**: Multi-stage Dockerfile (65MB runtime image) + Docker Compose with health checks
8. **Data Isolation**: JPA entity relationship between users and URLs, ownership-verified queries

### Interview Talking Points

**Q: How does the cache-aside pattern work?**
> When a request comes in for a short URL, the system first checks Redis. On a cache hit, the original URL is returned in ~1ms. On a miss, MySQL is queried (~10ms), the result is written to Redis with the appropriate TTL, and future requests hit the cache. On URL updates, the cache is invalidated and refreshed immediately.

**Q: How do you prevent users from seeing each other's data?**
> The `UrlMapping` entity has a `@ManyToOne` relationship with `User`. All repository queries include a `WHERE u.user.id = :userId` clause. Every service method that accesses a URL by short code calls `verifyOwnership()` to ensure the authenticated user owns the URL. Non-owned URLs return 404 (hiding existence).

**Q: Explain the rate limiting implementation.**
> Rate limiting uses a sliding window algorithm. Each request creates a Redis sorted set keyed by `rate_limit:{IP}:{group}`. A Lua script atomically: (1) removes entries outside the window, (2) counts remaining entries, (3) if count ≥ threshold returns deny with retry-after seconds, otherwise (4) adds the current timestamp and returns allow. The Lua script ensures atomicity — critical for concurrent requests.

**Q: How does token refresh work?**
> Access tokens expire after 24 hours. The frontend checks `token.exp` before each API call. If expiring within 60 seconds, it pre-emptively calls `/api/v1/auth/refresh` with the refresh token. On 401 responses, it retries once after refresh. Refresh tokens expire after 7 days and are one-time use. Both tokens are stored in localStorage and cleared on logout.
